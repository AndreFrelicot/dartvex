...
void main() {
  group('CupertinoWebSocketAdapter contract', () {
    ...
    test('connect resolves and reports connected', () async {
      ...
      expect(
        connectedUrls.single.toString(),
        'wss://example.convex.cloud/sync',
      );
    });
    ...
    test('a binary frame with malformed UTF-8 surfaces as a replacement '
      ...
      expect(
        ...
        isEmpty,
        reason:
            'malformed network input must never escape the adapter as an '
            'uncaught zone error',
      );
      ...
      expect(
        ...
        contains('�'),
        reason:
            'malformed bytes decode with replacement characters; the '
    ...
    test('a socket event stream error surfaces as a close event', () async {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'socket stream errors must reach the sync layer as close '
    ...
    test(
      'send forwards to the socket; throws StateError when disconnected',
      () async {
    ...
    test('remote close clears isConnected before delivering the close event '
      ...
      expect(
        ...
        isFalse,
        reason:
            'adapter contract: isConnected must be false by the time '
    ...
    test('close() closes the socket with a clean 1000 and emits one close '
    ...
    test(
      'a connect superseded while in flight discards the late socket',
      () async {
    ...
    test(
      'reconnect after remote close works and dedupes close events',
      () async {
        await adapter.connect('wss://example.convex.cloud/sync');
        final closes = <WebSocketCloseEvent>[];
        final sub = adapter.closeEvents.listen(closes.add);
