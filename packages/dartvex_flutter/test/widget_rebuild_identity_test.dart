...
void main() {
  ...
  testWidgets(
      'PaginatedQueryBuilder keeps loaded pages when a parent rebuild '
  ...
  testWidgets(
    ...
    expect(client.paginatedQueryCalls, hasLength(1));
    expect(capturedItems, <String>['b:1'],
  ...
  testWidgets(
      ...
      'new decode closure', (tester) async {
    final client = connectedClient();
  ...
  testWidgets('ConvexQuery re-decodes the current value when decode changes',
    ...
    expect(client.subscribeCalls, hasLength(1));
    expect(capturedSnapshot?.data, 'b:x',
  ...
  testWidgets(
      ...
      'decode and optimisticUpdate closures', (tester) async {
    final client = connectedClient();
